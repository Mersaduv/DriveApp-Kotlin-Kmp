...
fun MyApplicationTheme(
    ...
    languageManager: AppLanguageManager = LocalLanguageManager.current,
    content: @Composable () -> Unit
) {
    val colors = if (darkTheme) {
        darkColorScheme(
            primary = Color(0xFF6764F0), // A purple shade that works well for the app
            secondary = Color(0xFF03DAC5),
            tertiary = Color(0xFF3700B3)
        )
    } else {
        lightColorScheme(
            primary = Color(0xFF6764F0), // Same primary color for light theme
            secondary = Color(0xFF03DAC5),
            tertiary = Color(0xFF3700B3)
        )
    }
